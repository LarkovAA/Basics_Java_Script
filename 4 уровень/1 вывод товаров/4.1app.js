...
let button_el = document.querySelectorAll('button')


button_el.forEach(el => {
...
function showCategory(category) {
    let products_clera = document.querySelector('.products')
    console.log(products[category])
    let text = getProductMarkup(products[category])
    products_clera.insertAdjacentHTML('afterbegin', text)
}
...
function getProductMarkup(product) {
    ...
    product.forEach(el => {
        console.log(el)
        text = text + `<div>${el['name']}</div><img src="${el['imageUrl']}" alt=""><div>${el['price']}</div > <a href="https://example.com/producs/${el['id']}">Подробнее</a></div > `
    })
    return text
}
