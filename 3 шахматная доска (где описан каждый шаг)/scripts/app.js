let app = {
    ...
    generateBoard() {
        ...
        for (let i = 0; i < 8; i++) {    // Почему то config.rows.length выдает ошибку
    ...
    generateRow(startWithColor, rowNum) {
        ...
        for (let i = 0; i < 8; i++) { // config.cols.length то же самое ошитбка
            //let el = config.cols[i]
